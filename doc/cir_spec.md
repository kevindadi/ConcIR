...
Required top-level fields are `program`, `resources`, `protection`, `functions`, and `entry`. `fn_summaries` and `goals` default to empty arrays when omitted.
...
- `FnSummary`: `{ "name", "reads", "writes", "callees", "has_concurrency" }`; all five fields are required.
